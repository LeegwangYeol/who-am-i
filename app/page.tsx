...
export default function ProfilePage() {
  ...
  return (
    <div className={`min-h-screen ${isDarkTheme ? 'bg-gray-900' : 'bg-gray-200'} relative`}>
      ...
      <header className={`bg-white ${isDarkTheme ? 'bg-gray-800' : 'bg-white'} shadow`}>
        <div className="max-w-7xl mx-auto py-6 px-4 sm:px-6 lg:px-8">
          <h1 className={`text-3xl font-bold b ${isDarkTheme ? 'text-gray-200' : 'text-gray-900'}`}>
            이광열 <span className={`text-xl ${isDarkTheme ? 'text-gray-400' : 'text-gray-600'}`}>/ Lee Gwang Yeol</span>
          </h1>
        </div>
      </header>
...
📚 지속적인 학습과 🚀 도전을 통해 기술 혁신을 추구합니다!
      ...
      {showScrollTop && (
        <button
          onClick={scrollToTop}
          className={`${isDarkTheme ? 'glassmorphism-dark' : 'glassmorphism-light'} ${isDarkTheme ? 'text-white' : 'text-black'} fixed bottom-8 right-1/2 z-50 translate-x-1/2 bg-blue-500 text-white p-2 rounded-full shadow-lg hover:bg-blue-600 transition-colors`}
