...
const HighlightedText = ({ text, highlights }: { text: string, highlights: string[] }) => {
  ...
  return (
    <>
      {parts.map((part, i) => 
        part.highlight 
          ? <strong key={i} className="text-blue-600">{part.text}</strong>
    ...
    <div className="bg-white border border-gray-200 rounded-md mb-6">
      <div className="p-4 flex items-center">
        ...
        <div className="ml-3">
          ...
          <p className="text-xs text-gray-500">{category}: {categoryValue}</p>
        </div>
      </div>
      ...
      <div className="p-4">
        <div className="flex justify-between mb-2">
          <div className="flex space-x-4">
            <button onClick={() => onLike(likeKey)} className="flex items-center">
              <Heart className={`h-6 w-6 ${likes[likeKey] > defaultLikes[likeKey] ? "fill-red-500 text-red-500" : ""}`} />
            </button>
        ...
        <p className="font-semibold mb-1">{likes[likeKey]} likes</p>
